...
InputManager::InputManager( void ) :
    ...
    mKeyboard( 0 ),
    mInputSystem( 0 ),
	mKeyPressed( false ), mMouseButtonPressed ( false ), mJoystickButtonPressed ( false ),
	mLastKeyPressed(OIS::KC_UNASSIGNED)
	{
}
...
void InputManager::removeKeyListener( OIS::KeyListener *keyListener ) {
	std::map<std::string, OIS::KeyListener*> temp = mKeyListeners;
    std::map<std::string, OIS::KeyListener*>::iterator itKeyListener = temp.begin();
	std::map<std::string, OIS::KeyListener*>::iterator itKeyListenerEnd = temp.end();
    for(; itKeyListener != itKeyListenerEnd; ++itKeyListener ) {
        if( itKeyListener->second == keyListener ) {
            temp.erase( itKeyListener );
			mKeyListeners = temp;
            break;
        }
    }
...
int InputManager::getNumOfJoysticks( void ) const {
    // Cast to keep compiler happy ^^
    return (int) mJoysticks.size();
}

bool InputManager::anyButton( void ) const
{
	return (mMouseButtonPressed || mJoystickButtonPressed );
}

bool InputManager::anyKey( void ) const
{
	return mKeyPressed;
}
...
bool InputManager::isButtonDown(int joystick, int button) const
{
	if(joystick < getNumOfJoysticks())
	{
		const OIS::JoyStickState& state = mJoysticks[joystick]->getJoyStickState();
		return state.buttonDown(button);
	}
	return false;
}

bool InputManager::isPovDown(int joystick, int axis, int pov) const
{
	if(joystick < getNumOfJoysticks())
	{
		const OIS::JoyStickState& state = mJoysticks[joystick]->getJoyStickState();
		return state.mPOV[axis].direction == pov ? true : false;
	}
	return false;
}

bool InputManager::isSliderDown(int joystick, int axis) const
{
	if(joystick < getNumOfJoysticks())
	{
		const OIS::JoyStickState& state = mJoysticks[joystick]->getJoyStickState();
		return (state.mSliders[axis].abX != 0 || state.mSliders[axis].abY != 0);
	}
	return false;
}

int InputManager::getAxisValue(int joystick, int axis) const
{
	if(joystick < getNumOfJoysticks())
	{
		const OIS::JoyStickState& state = mJoysticks[joystick]->getJoyStickState();
...
// ----------------------------------------------------------------------------
void InputManager::clearPreviousStates()
{
	mKeyPressed = false;
	mMouseButtonPressed = false;
	mJoystickButtonPressed = false;
	mLastKeyPressed = OIS::KC_UNASSIGNED;
}
